...
## `filter()`
select() looks at columns, filter() looks within rows!!
...
#below is just me trying out something
filter(fish, length >400, scalelength > 11, radii_length_mm > 8, lakeid == "WS")
```
...
```{r}
names(mammals)
interest <- select(mammals, "family", "genus", "species", "mass", "gestation")
interest
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
filter(interest, family == "Bovidae" & mass > 450000)
```
...
```{r}
names(mammals)
values <- select(mammals, genus, mass, gestation, newborn)
primate_genera <- filter(values, genus %in% c("Lophocebus", "Erythrocebus", "Macaca"))
new_df <- data.frame(primate_genera)
new_df
filter(new_df, newborn == min(newborn))
```
